<x-admin-layout>
    ...
    <div class="content-wrapper container-xxl p-0">
        ...
        <div class="content-body">
            ...
            @include('backend.capacitaciones.from')
            @include('backend.capacitaciones.table')
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            var datatable = "{{ route('datatableCapacitaciones') }}";
              var saveFormulario = "{{ route('update.seccioncapacitacion') }}";
            var saveFormularioItem = "{{ route('nosotros_item.save') }}";
            var updateFormularioItem = "{{ route('nosotros_item.update') }}";
            var deleteFormularioItem = "{{ url('capacitaciones.destroy') }}";

        </script>
        @include('layouts.back_front.scripts_datatable')
        <script src="{{ asset('app-assets/vendors/js/pickers/flatpickr/flatpickr.min.js') }}"></script>
        <script src="{{ asset('app-assets/pages/capacitaciones_table.js') }}"></script>
    @endpush
</x-admin-layout>
