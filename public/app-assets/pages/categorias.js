...
$(document).ready(function() {
    ...
    $('#saveCategoria').on('click', function() {
        ...
        if (idAbout == 0){
            guardarAboutItem(token,titulo,subtitulo,table);
        }else{
            actualizarAboutItem(token,titulo,subtitulo,idAbout,table);

        }
    });
});

function guardarAboutItem(token,nombre_menu,subtitulo, table) {
    $.ajax({
        url: saveFormulario, // Asegúrate de que la URL es correcta
        type: 'POST',
        headers: {
            'X-CSRF-TOKEN': token
        },
        ...
        success: function(response) {
            ...
            $('#categoriasCursosForm')[0].reset();

            // Cerrar el modal
            $('#categoriasCursos').modal('hide');
        },
...
function actualizarAboutItem(token,nombre_menu,subtitulo,idSeccion,table){
    $.ajax({
    ...
    window.openEditModal = function(id) {
        fetch('/categoriaCurso/' + id + '/edit')
            ...
            .then(data => {
                ...
                var navModal = new bootstrap.Modal(document.getElementById('categoriasCursos'));
                navModal.show();
            })
